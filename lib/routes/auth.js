...
const User = require('../models/register-plugins');
...
router
  ...
  .put('/users/:id/roles/:role', ensureAuth(), ensureRole(), (req, res, next) => {
    const object = { 
      $addToSet: { 
        roles: req.params.role 
      } 
    };
    User.findByIdAndUpdate(req.params.id, object)
      .then(user => res.json(user))
      .catch(next);
  });

  // .delete('/api/auth/users/:id/roles/:role/', ({ user, params }, res, next) => {
  //   User.updateById(user.id, {
  //     $pull: {
  //       roles: params.user.id
  //     }
  //   })
  ...
  // .get('/api/auth/users', (req, res, next) => {
  //   User.find()
  //     .populate('roles', 'role')
  //     .then(({ roles }) => res.json(roles))
  //     .catch(next);
